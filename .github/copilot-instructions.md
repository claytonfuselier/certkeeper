...
- **Agent-facing routes** (`/api/agent`) are mTLS-authed:
  ...
  - `POST /api/agent/heartbeat` — keepalive with config versioning. Returns deployment count, `deployments_hash` (SHA-256 of deployment state — IDs, enabled flags, cert IDs, content hashes — truncated to 16 hex chars; `null` if no deployments), cert expiry, `heartbeat_interval` (seconds), `config_version`, and `actions` array (server→agent commands). Agent sends `{ config_version }` in request body to acknowledge settings. Agent compares `deployments_hash` with its locally stored value to detect changes without polling the full deployments list.
...
- Continue building out and testing notification options, including SMTP setup for email.
